...
await payee.click();
await payee.fill('');
await payee.pressSequentially('מוטב E2E', { delay: 15 });
...
console.log('transfer visible after save', await page.locator('#transfer-bank-number').isVisible().catch(() => false));
console.log('modal error', await page.locator('.modal-overlay').last().locator('.error').textContent().catch(() => ''));

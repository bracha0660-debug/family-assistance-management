...
public static class PaymentsEndpoints
{
    public static void MapPaymentsEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/payments/{id:guid}/execute", Execute).RequirePermission(PermissionKeys.PaymentsExecute);
        group.MapPost("/payments/{id:guid}/proof", UploadProof).RequirePermission(PermissionKeys.PaymentsUploadProof);
        group.MapPatch("/payments/{id:guid}/mark-paid", MarkPaid).RequirePermission(PermissionKeys.PaymentsMarkPaid);
        group.MapPost("/payments/{id:guid}/return-to-coordinator", ReturnToCoordinator).RequirePermission(PermissionKeys.PaymentsReturnToCoordinator);
    }
    ...
    private static Guid GetUserId(HttpContext httpContext) =>
        httpContext.GetAuthorizationContext()!.UserId;
    ...
    {
        var result = await service.ListQueueAsync(GetOrgId(httpContext), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        var result = await service.ExecuteAsync(
            GetOrgId(httpContext), id, request, ReadIfMatch(httpContext), GetUserId(httpContext), cancellationToken);
        if (!result.IsSuccess)
            return ToError(result);
        return Results.Ok(new PaymentResponse { Payment = result.Value! });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await service.UploadProofAsync(
            GetOrgId(httpContext), id, metadata, stream, ReadIfMatch(httpContext), GetUserId(httpContext), cancellationToken);
        if (!result.IsSuccess)
            return ToError(result);
        return Results.Ok(new PaymentResponse { Payment = result.Value! });
    }
